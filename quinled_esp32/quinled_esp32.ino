/*
 * SlyLED — QuinLED Dig-Uno (ESP8266 / ESP32)
 * Simple test sketch: WiFi web UI to turn a small rainbow strip on/off.
 *
 * Supports: ESP8266 (default), ESP32
 * - Board: WiFi Kit 8 / NodeMCU (ESP8266) or ESP32 Dev Module (ESP32)
 * - Data pin: GPIO2
 ...
 *
 * WiFi credentials in quinled_esp32/arduino_secrets.h (not tracked in git).
 */
...
WebServer server(80);
#else
#error "Unsupported platform. Select ESP8266 or ESP32 board."
#endif
...
void handleRoot() {
  ...
  html += F(
    ...
    "</head><body>"
    "<h1>SlyLED</h1>"
    "<p>QuinLED Dig-Uno — test strip on GPIO2, 8 LEDs.</p>"
