...
| --- | --- | --- |
| **Red banner: "Another SlyLED orchestrator is running on this network"** | Every tab shows a red banner naming another orchestrator (hostname, IP, version, a link to it); the Android connection pill reads "2 orchestrators"; `/status` lists it under `peerOrchestrators`. | Two orchestrators on one lighting network fight over the same devices (DMX / sACN output, HinksPix pushes, performers, gyro claims, the scheduler). **Stop one of them** — both instances show the banner, and it clears about 90 s after the other one stops. "Acknowledge for this session" only shrinks it. Detection needs both on the same broadcast domain; for an orchestrator on another VLAN, set `SLYLED_PEER_TARGETS=host[,host…]` on each (added in v2.2.1, #966). |
