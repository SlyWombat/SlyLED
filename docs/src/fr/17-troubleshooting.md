...
| --- | --- | --- |
| **Bandeau rouge : « Another SlyLED orchestrator is running on this network »** | Chaque onglet affiche un bandeau rouge nommant un autre orchestrateur (nom d'hôte, IP, version, lien) ; la pastille de connexion Android indique « 2 orchestrators » ; `/status` le liste dans `peerOrchestrators`. | Deux orchestrateurs sur le même réseau d'éclairage se disputent les mêmes appareils (sortie DMX / sACN, envois HinksPix, performers, gyroscopes, planificateur). **Arrêtez l'un des deux** — les deux instances affichent le bandeau, qui disparaît environ 90 s après l'arrêt de l'autre. « Acknowledge for this session » ne fait que le réduire. La détection exige le même domaine de diffusion ; pour un orchestrateur sur un autre VLAN, définissez `SLYLED_PEER_TARGETS=hôte[,hôte…]` sur chacun (ajouté en v2.2.1, #966). |
