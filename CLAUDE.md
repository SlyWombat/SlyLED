...
| 0x80 | ORCH_ANNOUNCE | orchestrator→broadcast | ≥8 bytes — instanceId(u32) + httpPort(u16) + verLen(u8)+version (≤32) + hostLen(u8)+hostname (≤64). Sent every 30 s (and back to a newly seen peer) so a second orchestrator on the LAN is detected on both sides (#966); a CMD_PING from a foreign address also counts. Firmware ignores it; UDP_VERSION stays 5. 0x80+ is the orchestrator↔orchestrator range (not in the firmware headers). Source: `desktop/shared/peer_orchestrators.py` |
