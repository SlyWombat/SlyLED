...
| `qa_isolated_env.py` | `up`/`smoke`/`down` an **isolated** Docker `--internal` network on kdocker3 (two orchestrators + fake HinksPix + runner, 10.250.0.0/24) — no LAN/production contact; use for anything that would otherwise be a second orchestrator on the lighting LAN (#966) |
| `fake_hinkspix.py` | Standalone fake HinksPix MS_160 (captured replies, gzipped CGI, ROW 906/907, E1.31 receive counters) |
